          ...
          <h2 class="title">Shopping Cart</h2>
            <div class="table-responsive form-group">
              <table class="table table-bordered">
                	...
                	if(count($mycart)>=1)
                	{
                	 //print_r($cart);
                	 	foreach ($mycart as $key) {
                	 			$items=unserialize($key->data);
                	 			//print_r($items);

                	 			foreach ($items as $ckey) {

                	 		//print_r($cart)
                 ?>

                  <tr>
                    <td class="text-center"><a href="product.html"><img width="100px" src="<?php echo base_url() ?>upload/products/<?php echo $ckey['ImageLogo'] ?>" alt="<?php echo $ckey["productTitle"] ?>" title="<?php echo $ckey["productTitle"] ?>" class="img-thumbnail" /></a></td>
                    <td class="text-left"><a href="product.html"><?php echo $ckey['productTitle'] ?></a><br />
                     </td>

                    <td class="text-left" width="200px"><div class="input-group btn-block quantity">
                        <input type="text" name="quantity" id="quantity" value="<?php echo $key->quantity ?>" size="" class="form-control" />
                        <span class="input-group-btn">
                        <?php $val=$ckey['salePrice']; ?>
                        <button type="submit" data-toggle="tooltip" title="Update" class="btn btn-primary" onclick="ucart.update('<?php echo $key->cartid ?>',document.getElementById('quantity').value,'<?php echo $val; ?>')"><i class="fa fa-clone"></i></button>

                        <button type="button" data-toggle="tooltip" title="Remove" class="btn btn-danger" onclick="rcart.remove('<?php echo $key->cartid ?>')"><i class="fa fa-times-circle"></i></button>
                        </span></div></td>
                    <td class="text-right"><i class="fa fa-rupee"></i><?php echo $ckey['salePrice']; ?>.00 </td>

                    <td class="text-right"><i class="fa fa-rupee"></i>
                     <?php

                     $q= $key->quantity ;
                     $orgtotal +=($ckey['price'])*$q;
                     $distotal +=($ckey['price']-$ckey['salePrice'])*$q;

                     	$grandtotal +=$key->amount;
                     echo $key->amount?>.00</td>
                  </tr>
                ...
                <tr>
                	<td colspan="5"><p align="center">Your Shopping Cart is Empty!</p></td>
                </tr>
                <?php } ?>
                  </tbody>
              </table>
				...
				<table class="table table-bordered">
					<tbody>
						<tr>
							...
							<td class="text-right"><i class="fa fa-rupee"></i><?php echo $orgtotal; ?></td>
						</tr>
						<tr>
							<td class="text-right">
								<strong>Total Discount (-):</strong>
							</td>
							<td class="text-right"><i class="fa fa-rupee"></i><?php echo $distotal; ?>.00</td>
						</tr>
						<tr>
							<td class="text-right">
							...
							<?php if($distotal<100){$grandtotal +=50;?>50
									<?php }elseif($distotal>200){$grandtotal+=100;?>100
									<?php }elseif($distotal>300){$grandtotal+=150;?>150
									<?php }elseif($distotal>400){$grandtotal+=200;?>200
									<?php }elseif($distotal>500){$grandtotal+=250;?>250
									<?php }else{ ?>0<?php }?>
						...
						<tr>
							...
							<td class="text-right"><i class="fa fa-rupee"></i><?php echo $grandtotal ?>.00</td>
						</tr>
					</tbody>
				</table>
			</div>
