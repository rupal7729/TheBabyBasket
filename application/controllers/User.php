  ...
  public function registration()
  {
    $this->load->view('user_reg');
     if($this->input->post("reg"))
                ...
                if($c>0)
                {
                    echo '<script language="javascript">alert("email id is already Exist!!please try Another !!!")</script>';
  ...
  public function email($email)
   {
     ...
     public function myaccount()
     {
      ...
      $data['statedata']=$this->User_model->select("tblState");
      $data['citydata']=$this->User_model->select("tblCity");
     ...
     // print_r($uid);
      // $wh=array("tblUser.userid"=>$uid);
       $wh="userid='".$uid."'";
         ...
         $data["check"]=$this->User_model->select_wh('tblAddress',$wh);
         $data["check1"]=$this->User_model->select_wh('tblChild',$wh);
         $c=count($data["check"]);
         $c1=count($data["check1"]);
         //print_r($c);
         if($c>=1)
         {
             $wh="tblUser.userid='".$uid."'";

            $arr=array("tblAddress"=>'tblAddress.userid=tblUser.userid',"tblState"=>'tblState.stateid=tblAddress.state',"tblCity"=>'tblCity.cityid=tblAddress.city');
             $data['adata']=$this->User_model->select_join_wh('tblUser',$arr,$wh);
         }
         else
         {
            $data['adata']=$this->User_model->select_wh('tblUser',$wh);
         }
         ...
         if($c1>=1)
         {
            ...
            $arr=array("tblChild"=>'tblChild.userid=tblUser.userid');
             $data['cdata']=$this->User_model->select_join_wh('tblUser',$arr,$wh);
         }
         else
         {
            $data['adata']=$this->User_model->select_wh('tblUser',$wh);
        ...
        if(isset($_GET['childid']))
        {
          $c=$_GET['childid'];
              if(!empty($c))
              {
                $wh=array("childid"=>$c);
              print_r($c);
              $this->User_model->delete_data("tblChild",$wh);
                if($this->db->affected_rows()>=1)
                {
...
//SELECT round(avg(`rating`),2) AS `average_rate`, count(`rating`) AS `num_of_rating` FROM tblReview WHERE productid=15
     ...
     public function wishlist()
     {
      ...
      $data["brand"]=$this->User_model->select("tblBrand");
        ...
        $uid= $id[0]->userid;
        $wh=array("tblUser.userid" => $uid);
        $arr=array("tblUser"=>'tblUser.userid=tblWishlist.userid',"tblProduct"=>'tblProduct.productid=tblWishlist.productid');
        $data["wish"]=$this->User_model->select_join_wh("tblWishlist",$arr,$wh);
        print_r($data["wish"]);
      $this->load->view('wishlist',$data);
     }


     public function removerwishlist()
     {
        if(isset($_GET['rid']))
        {
          $rid=$_GET['rid'];
          $wh=array("productid"=>$rid);
          $this->User_model->delete_data("tblWishlist",$wh);
          if($this->db->affected_rows()>1)
          {
            echo "success";
          }
     ...
     public function removecart()
    {
          if(isset($_POST['action']))
          {
              ...
              $cid=$_POST['cartid'];
                  $wh=array('cartid'=>$cid);
                  $this->User_model->delete_data("tblCart",$wh);
                  //print_r($wh);
                  if($this->db->affected_rows()>1)
                  {
                      echo 'success';
...
//SELECT * FROM tblProduct WHERE color LIKE '%Blue'
   ...
   public function addtocart()
   {
      ...
      if(isset($_POST["action"]))
      {
         if(isset($_POST["productid"]))
         {
           ...
           if(!empty($uid) && !empty($p))
           {
              ...
              $check=$this->User_model->select_wh("tblCart",$wh);
              if(count($check)>=1)
              {
                echo "already there";
              }
     ...
     // print_r($sa);
         ...
         $ins=array("userid"=>$uid,"productid"=>$p,"amount"=>$totalamt,"quantity"=>$qun,"data"=>$sa);
              print_r($ins);
 ...
 public function shoppingcart()
  ...
  public function updatecart()
  {
    if(isset($_POST['action']))
    {
      ...
      $totalamt=$amt*$quantity;
      //print_r($quantity);]
      $wh=array("cartid"=>$cartid);
      $arr=array("quantity"=>$quantity,"amount"=>$totalamt);
      print_r($arr);
      $this->User_model->update("tblCart",$arr,$wh);
      if($this->db->affected_rows()>1)
      {
         echo "success";
      }
